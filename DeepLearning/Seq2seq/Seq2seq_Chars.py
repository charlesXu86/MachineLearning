...
def seq2seq_model(input_data, targets, lr, target_sequence_length, max_target_sequence_length, source_sequence_length,
    ...
    # 将状态向量与输入传递给decoder
    training_decoder_output, predictor_decoder_output = decoding_layer(target_letter_to_int, decoder_embedding_size, num_layers, rnn_size, target_sequence_length,
                                                                       max_target_sequence_length, encoder_state, decoder_input)
    return training_decoder_output, predictor_decoder_output
...
with train_graph.as_default():
    ...
    training_decoder_output, pridicting_decoder_output = seq2seq_model(
                                                                       input_data,
                                                                       targets, lr, target_sequence_length, max_target_sequence_length, source_sequence_length, len(source_letter_to_int),
                                                                       len(target_letter_to_int), encoding_embedding_size, decoding_embedding_size, rnn_size, num_layers
                                                                        )
    training_logits = tf.identity(training_decoder_output.rnn_output, 'logits')
    predicting_logits = tf.identity(pridicting_decoder_output.sample_id, name='predictions')
