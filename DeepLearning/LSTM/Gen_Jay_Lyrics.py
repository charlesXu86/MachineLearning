...
#             self.cell = tf.nn.rnn_cell.BasicLSTMCell(args.state_size)
#             self.cell = tf.nn.rnn_cell.MultiRNNCell([self.cell] * args.num_layers)
...
#
#     args = HParam()
#     data = DataGenerator('F:\project\MachineLearning\DeepLearning\LSTM\data\JayLyrics.txt', args)
...
def train(data, model, args):
    with tf.Session() as sess:
        ...
        saver = tf.train.Saver()
        # 有修改
        # writer = tf.train.SummaryWriter(args.log_dir, sess.graph)  已经废除
        writer = tf.summary.FileWriter(args.log_dir, sess.graph)
        # Add embedding tensorboard visualization. Need tensorflow version
        # >= 0.12.0RC0
        config = projector.ProjectorConfig()
