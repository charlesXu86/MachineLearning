...
def update(f):
    ...
    if save_image:
        if f % 3 == 0:
            ...
            output =  'F:\project\MachineLearning\MachineLearning\Data\Output\\NaiveBayes' #'.\\Pic2\\'
...
def calc_next_loc(now, loc, directions):
    ...
    ii[ii >= m] = 0
    jj[jj >= n] = 0
    return np.dot(loc[ii, jj], directions[ii, jj, directions_index])   # dot 两个数组的点积
...
if __name__=='__main__':
    ...
    if style == 'Direct':
        ...
        theta = np.empty_like(x, dtype=np.int)  # 返回具有与给定数组相同的形状和类型的数组，但是不初始化返回的数组
        theta[y_d > 0.5] = 1
